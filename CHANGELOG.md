...
### Added
- **Always-visible edit/delete buttons on MealCard**: removed long-press-only reveal for mobile; edit (pencil) and delete (trash) icons are now permanently visible in the meal card header for all screen sizes.
- **Delete confirmation dialog on MealCard**: tapping the trash icon now shows a modal with the meal name, a Cancel button, and a destructive Delete button before the meal is removed. New i18n key `deleteMealConfirm` added in all 7 languages.
- **Edit meal via chat**: users can now ask the assistant to correct or update their last logged entry (e.g. "I made a mistake in my last meal", "edit my last entry"). `classifyIntent` now returns three values: `'log' | 'question' | 'edit'`. When `'edit'` is detected, Chat fetches the most recent meal from today or yesterday and shows an inline `EditMealCard` pre-populated with the meal's description, food items and macros. Saving writes directly to `db.meals.update()` and marks the card as read-only. If no recent meal is found, an error bubble is shown instead. New `EditMealCard` sub-component added above `Chat`. New i18n keys: `chatEditMealFound`, `chatEditMealNotFound`, `chatEditMealSaved` in all 7 languages. New `ChatMessage` union variant `{ role: 'edit-meal'; meal: MealEntry; saved?: boolean }`.
...
- **Article: "Where the Heck Do I Get All That Protein?"** (`slug: where-to-get-protein`): 4-min read, 🥩. Covers why protein is hard to hit, a table of top sources (chicken, Greek yogurt, eggs, cottage cheese, tuna, salmon, tempeh, lentils, whey), practical strategies, and plant-based tips. Now available in all 7 languages (en/ru/uk/cs/de/fr/es).
- **Article: "How Much to Eat to Feel Good (Without Overeating)"** (`slug: how-much-to-eat`): 5-min read, 🍽️. Covers the 20-min satiety lag, what makes food filling (protein, fibre, volume, water content), practical stop-at-the-right-moment habits, calorie-density visual bar chart, and the plate method. Now available in all 7 languages (en/ru/uk/cs/de/fr/es).
- **Full article translations**: added cs/de/fr/es translations for `where-to-get-protein` and `how-much-to-eat` articles; all 6 articles in the library now have complete coverage across all 7 supported languages.
...
### Added
- **App renamed to "Eat Right"**: updated in HTML title, Open Graph / Twitter meta tags, PWA manifest name, apple-mobile-web-app-title, JSON-LD structured data, and all 7 onboarding welcome strings.
